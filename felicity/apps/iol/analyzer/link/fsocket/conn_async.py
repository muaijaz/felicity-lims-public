# -*- coding: utf-8 -*-
"""
Async Socket Link - Non-blocking instrument communication
...
Architecture:
- AsyncSocketLink: Main connection handler
...
class AsyncSocketLink(AbstractLink):
    """
    Async TCP/IP socket connection handler for laboratory instruments.
    ...
    async def start_server(self, trials: int = 1):
        ...
        if not self.is_active:
            logger.info(f"AsyncSocketLink {self.name}: Instrument deactivated, cannot start server")
            return
        ...
        try:
            ...
            logger.error(f"AsyncSocketLink {self.name}: Error starting server: {e}")
            if self.emit_events:
    ...
    async def _start_server_mode(self):
        ...
        """
        logger.info(f"AsyncSocketLink {self.name}: Starting server mode on {self.host}:{self.port}")
        ...
        try:
            ...
            logger.info(f"AsyncSocketLink {self.name}: Server listening on {self.host}:{self.port}")
            ...
            if e.errno in (98, 13):  # Address in use or Permission denied
                logger.error(f"AsyncSocketLink {self.name}: Port error ({e.errno}): {e}")
            else:
                raise
        except asyncio.CancelledError:
            logger.info(f"AsyncSocketLink {self.name}: Server shutdown")
        except Exception as e:
            logger.error(f"AsyncSocketLink {self.name}: Server error: {e}")
    ...
    async def _handle_client(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        logger.info(f"AsyncSocketLink {self.name}: Client connected from {addr}")
        ...
        try:
            ...
            while True:
                try:
                    ...
                    logger.warning(f"AsyncSocketLink {self.name}: Read timeout from {addr}")
                    break

                if not data:
                    logger.info(f"AsyncSocketLink {self.name}: Connection closed by {addr}")
                    break

                # Check message timeout
                if self._check_message_timeout():
                    logger.error(f"AsyncSocketLink {self.name}: Message timeout detected")
                ...
                if self._check_message_size(len(data)):
                    logger.error(f"AsyncSocketLink {self.name}: Message exceeds size limit")
                    response = self._encode_response("NACK")
                    writer.write(response)
                    await writer.drain()
                    self._close_session()
            ...
            logger.info(f"AsyncSocketLink {self.name}: Client handler cancelled")

        except Exception as e:
            logger.error(f"AsyncSocketLink {self.name}: Client error: {e}")
    ...
    async def _start_client_mode(self):
        ...
        """
        logger.info(f"AsyncSocketLink {self.name}: Starting client mode, connecting to {self.host}:{self.port}")
        ...
        while reconnect_count < MAX_RECONNECT_ATTEMPTS:
            try:
                ...
                logger.info(f"AsyncSocketLink {self.name}: Connected to {self.host}:{self.port}")
                ...
                try:
                    ...
                    while True:
                        try:
                            ...
                            logger.warning(f"AsyncSocketLink {self.name}: Read timeout")
                            break

                        if not data:
                            logger.info(f"AsyncSocketLink {self.name}: Server closed connection")
                            break

                        # Check timeout
                        if self._check_message_timeout():
                            logger.error(f"AsyncSocketLink {self.name}: Message timeout")
                            response = self._encode_response("NACK")
                            writer.write(response)
                            await writer.drain()
                            self._close_session()
                        ...
                        # Check size
                        if self._check_message_size(len(data)):
                            logger.error(f"AsyncSocketLink {self.name}: Message size limit exceeded")
                            response = self._encode_response("NACK")
                            writer.write(response)
                            await writer.drain()
                            self._close_session()
                ...
                logger.error(f"AsyncSocketLink {self.name}: Connection timeout")
                reconnect_count += 1

            except ConnectionRefusedError:
                logger.error(f"AsyncSocketLink {self.name}: Connection refused")
                reconnect_count += 1

            except Exception as e:
                logger.error(f"AsyncSocketLink {self.name}: Connection error: {e}")
                reconnect_count += 1

            if reconnect_count < MAX_RECONNECT_ATTEMPTS and self.auto_reconnect:
                logger.info(f"AsyncSocketLink {self.name}: Reconnecting... (attempt {reconnect_count}/{MAX_RECONNECT_ATTEMPTS})")
    ...
    def _check_message_timeout(self) -> bool:
        ...
        if elapsed > MESSAGE_TIMEOUT_SECONDS:
            logger.warning(f"AsyncSocketLink {self.name}: Message timeout after {elapsed:.1f}s")
            return True
        return False
    ...
    async def process(self, data: bytes) -> Optional[Union[str, bytes, tuple]]:
        ...
        logger.info(f"AsyncSocketLink {self.name}: Received {len(data)} bytes")
        ...
        if self.protocol_type is None and not self._protocol_detected:
            ...
            logger.info(f"AsyncSocketLink {self.name}: Detected protocol: {self._detected_protocol}")
        ...
        if protocol == ProtocolType.ASTM:
            ...
            logger.warning(f"AsyncSocketLink {self.name}: Unknown protocol, defaulting to ASTM")
            return await self.astm_handler.process_data(data)
    ...
    def __repr__(self):
        return (f"AsyncSocketLink(uid={self.uid}, name={self.name}, "
